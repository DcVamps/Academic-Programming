...
kvLED_ACTION_ALL_LEDS_ON = 0   # Turn all LEDs on.
kvLED_ACTION_ALL_LEDS_OFF = 1   # Turn all LEDs off.
kvLED_ACTION_LED_0_ON = 2   # Turn LED 0 on.
kvLED_ACTION_LED_0_OFF = 3   # Turn LED 0 off.
kvLED_ACTION_LED_1_ON = 4   # Turn LED 1 on.
kvLED_ACTION_LED_1_OFF = 5   # Turn LED 1 off.
kvLED_ACTION_LED_2_ON = 6   # Turn LED 2 on.
kvLED_ACTION_LED_2_OFF = 7   # Turn LED 2 off.
kvLED_ACTION_LED_3_ON = 8   # Turn LED 3 on.
kvLED_ACTION_LED_3_OFF = 9   # Turn LED 3 off.
...
class canlib(object):
    ...
    def __init__(self, debug=None):
        ...
        if sys.platform.startswith('win'):
            self.dll = ct.WinDLL(os.path.join(installDir, 'canlib32.dll'))
            self.dll.canInitializeLibrary()
        else:
            self.dll = ct.CDLL(os.path.join(installDir, 'libcanlib.so'))
        ...
        self.dll.canAccept.argtypes = [ct.c_int, ct.c_long, ct.c_uint]
        self.dll.canAccept.errcheck = self._canErrorCheck

        try:
            self.dll.canSetAcceptanceFilter.argtypes = [ct.c_int, ct.c_uint,
                                                        ct.c_uint, ct.c_int]
            self.dll.canSetAcceptanceFilter.errcheck = self._canErrorCheck
        ...
        try:
            self.dll.kvReadTimer.argtypes = [ct.c_int, ct.POINTER(ct.c_int)]
            self.dll.kvReadTimer.errcheck = self._canErrorCheck
        except AttributeError as e:
            logging.debug(str(e) + ' (Not implemented in Linux)')
        ...
        self.dll.kvFlashLeds.argtypes = [ct.c_int, ct.c_int, ct.c_int]
        self.dll.kvFlashLeds.errcheck = self._canErrorCheck
    ...
    def getChannelData_Cust_Name(self, channel):
    ...
    def getChannelData_Firmware(self, channel):
        ...
        Returns:
            fw_major (int): The major version number
            fw_minor (int): The minor version number
            fw_build (int): The build number
...
class canChannel(object):
    ...
    def canAccept(self, envelope, flag):
        ...
        Args:
            ...
            flag:      Any of the canFILTER_SET_xxx constants
                (CODE/MASK, STD/EXT)
        """
        self.fn = inspect.currentframe().f_code.co_name
        self.dll.canAccept(self.handle, envelope, flag)
    ...
    def canSetAcceptanceFilter(self, code, mask, is_extended=False):
        ...
        """
        self.fn = inspect.currentframe().f_code.co_name
        self.dll.canSetAcceptanceFilter(self.handle, code, mask, is_extended)
    ...
    def setBusParamsFd(self, freq_brs, tseg1_brs=0, tseg2_brs=0, sjw_brs=0):
        ...
        """
        self.canlib.fn = inspect.currentframe().f_code.co_name
        self.dll.canSetBusParamsFd(self.handle, freq_brs, tseg1_brs,
                                   tseg2_brs, sjw_brs)
    ...
    def write(self, id_, msg, flag=0, dlc=None):
        ...
        The variable name id (as used by canlib) is a built-in function in
        Python, so the name 'id\_' is used instead.
    ...
    def readTimer(self):
        ...
        self.canlib.fn = inspect.currentframe().f_code.co_name
        time = ct.c_int()
        self.dll.kvReadTimer(self.handle, time)
        return time.value
    ...
    def read(self, timeout=0):
        ...
        return(id_.value, bytearray(msg), dlc.value, flag.value,
               time.value)
    ...
    def flashLeds(self, action, timeout_ms):
        ...
        Args:
            action (int): One of the kvLED_ACTION_xxx constants, defining
        ...
        """
        self.canlib.fn = inspect.currentframe().f_code.co_name
        self.dll.kvFlashLeds(self.handle, action, timeout_ms)
    ...
    def getChannelData_Cust_Name(self):
        self.canlib.fn = inspect.currentframe().f_code.co_name
        try:
            return self.canlib.getChannelData_Cust_Name(self.index)
    ...
    def fileCopyFromDevice(self, deviceFileName, hostFileName):
        ...
        """
        self.canlib.fn = inspect.currentframe().f_code.co_name
        self.dll.kvFileCopyFromDevice(self.handle, deviceFileName.encode(),
                                      hostFileName.encode())
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        ...
        for ch in range(0, channels):
            try:
                print("%d. %s (%s / %s) Channel Name: '%s'" % (ch, cl.getChannelData_Name(ch),
                                            cl.getChannelData_EAN(ch),
                                            cl.getChannelData_Serial(ch),
                                            cl.getChannelData_Cust_Name(ch)))
            except (canError) as ex:
                #print(ex)  # It is ok to get error from getChannelData_Cust_Name, if not set in device.
    ...
    try:
        ...
        ch1.busOn()
        print("Using channel: %d. %s, EAN: (%s / %s), Channel Name: '%s'" % (ch, 
                      ch1.getChannelData_Name(),
                      ch1.getChannelData_EAN(),
                      ch1.getChannelData_Serial(),
                      ch1.getChannelData_Cust_Name()))
    except (canError) as ex:
        print(ex)
